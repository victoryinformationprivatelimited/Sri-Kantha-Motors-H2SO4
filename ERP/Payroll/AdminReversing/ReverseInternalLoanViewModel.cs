...
{
    class ReverseInternalLoanViewModel : ViewModelBase
    {
        #region Fields
        ...
        #region Constructor
        public ReverseInternalLoanViewModel()
        {
        ...
        private void Reverse()
        {
            try
            {
                if (Periods.LastOrDefault().period_id == CurrentPeriod.period_id)
                {
                    trns_ReverseLog trnsReverse = new trns_ReverseLog();
                    trnsReverse.reverse_category = CurrentPeriod.period_name + " Internal";
                    trnsReverse.reverse_user_id = clsSecurity.loggedUser.user_id;
                    trnsReverse.reverse_datetime = System.DateTime.Now;
                    List<trns_InternalLoanPayment> Exist = serviceClient.CheckToReverseInternalLoans(CurrentPeriod.period_id).ToList();
                    if (Exist.Count > 0)
                    {
                        if (serviceClient.SaveReverseLog(trnsReverse))
                        {
                            if (serviceClient.ReverseInternalBankLoans(CurrentPeriod.period_id))
                            {
                ...
                {
                    clsMessages.setMessage("Internal Loans can be Reversed For The Last Processed Period");
                }
            }
            catch (Exception)
            {
